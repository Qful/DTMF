...
BOOL PulwidthSetting()
{
	...
	if(CH367mWriteIoByte(mIndex,&mBaseAddr->mCH367Speed,CurPulWidth))//向CH367/CH368的速度寄存器中写入控制数据
	{
		...
		sprintf(PulShow,"%d\0",(x+1)*30);
		SetWindowText(GetDlgItem(mSaveDialogMain,IDC_PULSHOW),PulShow);//将设置的速度显示在主对话框上
	}
	else
	{
...
void mIoRead(HWND hDialog,int IoModel,int IoAddrType)//IO空间读
{
	...
	ULONG mLen=0,mAddr;
	mAddr=GetDlgItemText(hDialog,IDC_EDITIOADD,IoAddr,5);
	...
	mLen=GetDlgItemText(hDialog,IDC_EDITIOLEN,IoLen,5);	
	...
	if(GetDlgItemText(hDialog,IDC_EDITIOADD,IoAddr,5)==0||GetDlgItemText(hDialog,IDC_EDITIOLEN,IoLen,5)==0)
	{
...
void mIoWrite(HWND hDialog,int IoModel,int IoAddrType)//IO空间写
{
	...
	strLen=GetDlgItemText(hDialog,IDC_EDITIODATA,inputStr,mMAX_BUFFER_LENGTH);//得到输入框中的数据
	for(i=0;i<strLen;i++)
	{
		if(inputStr[i]!=' ')
			bufferFilter[j++]=inputStr[i];
	}
	if(GetDlgItemText(hDialog,IDC_EDITIOADD,IoAddr,5)==0||GetDlgItemText(hDialog,IDC_EDITIOLEN,IoLen,5)==0||GetDlgItemText(hDialog,IDC_EDITIODATA,IoLen,5)==0)
	{
...
void mI2CRead(HWND hDialog)//I2C读
{
	...
	UCHAR cByte;
	mVAR_TYPE mVarTyped;
	dAddrLen = GetDlgItemText(hDialog, IDC_EDITI2CADD, dAddr, 3); // 取数据单元地址 
	...
	if(dAddrLen) // 取数据单元地址 
	{
		if(CH367mReadI2C(mIndex, 0x50, idAddr, &cByte)) // 从I2C接口读一个字节 
		{
			...
			SetDlgItemText(hDialog, IDC_EDITI2CDATA, sByte);
		}
		else
		{
...
void mI2CWrite(HWND hDialog)
{
	...
	oByteLen = GetDlgItemText(hDialog, IDC_EDITI2CDATA, oByte, 3); // 取待写入数据 
...
LRESULT CALLBACK mDialogI2c(HWND hDialog, UINT uMessage, WPARAM wParam, LPARAM lParam)//I2C对话框处理函数
{
	switch(uMessage)
	{
...
void mConRead(HWND hDialog)//配置空间读
{
	...
	UINT iAddrLen;
	mVAR_TYPE mVarType;
	iAddrLen = GetDlgItemText(hDialog, IDC_EDITCONADD, iAddr, 5); // 取地址 
	...
	if(iAddrLen) 
	{
		if(CH367mReadConfig(mIndex, (PVOID)mAddr, &cByte)) /* 从配置空间读一个字节 */
		{
			UCHAR sByte[8];
			sprintf(sByte, "%02X\x0", cByte);
			SetDlgItemText(hDialog, IDC_EDITCONDATA, sByte);
		}
		else
		{
			MessageBox(hDialog, "读数据失败!", "提示", MB_OK | MB_ICONSTOP);
...
void mConWrite(HWND hDialog)//配置空间写
{
	...
	mVAR_TYPE mVarByte;
	int oByteLen = 0;
	iAddrLen = GetDlgItemText(hDialog, IDC_EDITCONADD, iAddr, 8); // 取地址 
	if(!mCheckInput(iAddr))
	{
		MessageBox(hDialog,"您输入的字符有误，请输入0~9,a~f,A~F之间的十六进制数!","提示",MB_OK|MB_ICONERROR);
		return;
	...
	mVarType = mCharToVar(iAddr,iAddrLen,0);
	mAddr = mVarType.sVar;	
	oByteLen=GetDlgItemText(hDialog, IDC_EDITCONDATA, oByte, 8); // 取数据 
	if(!mCheckInput(oByte))
	{
		MessageBox(hDialog,"您输入的字符有误，请输入0~9,a~f,A~F之间的十六进制数!","提示",MB_OK|MB_ICONERROR);
		return;
...
void read_flash(void) // 读FLASH数据写到文件 
{
	...
	{  
		...
		{  
			...
			{
				// 读FLASH 每次读256字节 
				if(read_block(address, SPI_BUFFER_LENGTH, p))
				{
